...
通常 test は mock endpoint で安定させます。real Ollama の任意 test を手元で動かす場合は `LOCAL_AI_RUN_REAL_OLLAMA_TESTS=1 LOCAL_AI_REAL_OLLAMA_HOSTS="<candidate-hosts>" node --test tests/ts_preflight.real_ollama.test.js` を使います。`LOCAL_AI_REAL_OLLAMA_HOSTS` を省く場合は `LOCAL_AI_OLLAMA_HOSTS` を使います。local endpoint が未応答なら `not_available` 扱いで、通常の verify 条件には含めません。
